...
exports.default = strapi_1.factories.createCoreController('api::fleet.fleet', ({ strapi }) => ({
    ...
    async incrementMileage(ctx) {
        ...
        const { increment = 500 } = ctx.request.body || {}; // Default 500 km (no 0.5)
        if (!documentId) {
        ...
        if (isNaN(incrementValue) || incrementValue <= 0) {
            return ctx.badRequest('El incremento debe ser un número positivo');
        }
        // Máximo 10,000 km por operación para prevenir errores
        if (incrementValue > 10000) {
            return ctx.badRequest('El incremento máximo permitido es 10,000 km');
        }
        try {
            // Buscar el vehículo
