...
exports.default = strapi_1.factories.createCoreService('api::fleet-reminder.fleet-reminder', ({ strapi }) => ({
     ...
     * Con protección contra duplicados (mismo recordatorio, mismo usuario, últimos 5 minutos)
     */
    async createMileageNotification(reminder, vehicle, currentMileage) {
        try {
            ...
            // Verificar si ya existe notificación reciente para evitar duplicados (últimos 5 minutos)
                ...
                try {
                    ...
                    const existingNotifications = await strapi.entityService.findMany('api::notification.notification', {
                        filters: {
                            ...
                            recipient: { id: { $eq: userId } },
                            fleetReminder: { id: { $eq: reminder.id } },
                            createdAt: { $gte: fiveMinutesAgo.toISOString() },
                        },
                        limit: 1,
                    });
                    if (existingNotifications && existingNotifications.length > 0) {
                        strapi.log.info(`[createMileageNotification] Notificación duplicada evitada para usuario ${userId}, recordatorio ${reminder.id}`);
                        continue;
                    }
            ...
            // También crear notificación general del vehículo (sin recipient)
            try {
                // Verificar duplicado general
                const existingGeneral = await strapi.entityService.findMany('api::notification.notification', {
                    filters: {
                        type: { $eq: 'oil_change_reminder' },
                        fleetReminder: { id: { $eq: reminder.id } },
                        recipient: { $null: true },
                ...
                if (existingGeneral && existingGeneral.length > 0) {
                    strapi.log.info(`[createMileageNotification] Notificación general duplicada evitada para recordatorio ${reminder.id}`);
                }
                else {
