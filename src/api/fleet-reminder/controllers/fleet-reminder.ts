...
export default factories.createCoreController('api::fleet-reminder.fleet-reminder', ({ strapi }) => ({
  async create(ctx) {
    ...
    // Sincronizar notificaciones después de crear el recordatorio
    if (result?.data) {
      try {
        ...
        // Log del error pero no interrumpir la creación
        strapi.log.error('Error sincronizando notificaciones después de crear recordatorio:', error);
      }
    }
  ...
  async update(ctx) {
    ...
    // Sincronizar notificaciones después de actualizar el recordatorio
    if (result?.data) {
      try {
        const notificationService = strapi.service('api::notification.notification');
        if (notificationService && typeof notificationService.syncReminderNotifications === 'function') {
        ...
        // Log del error pero no interrumpir la actualización
