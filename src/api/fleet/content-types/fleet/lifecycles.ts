...
export default {
  ...
  async beforeUpdate(event: {
    params: { data: FleetData; where: { documentId?: string; id?: number } };
  }) {
    ...
    if ((data.brand !== undefined || data.model !== undefined) && !data.billingInitials) {
      ...
      if (!documentId && where.id) {
        ...
        if (existing) {
          ...
          data.model = data.model ?? existing.model;
        }
      }
