...
describe('actionForMethod', () => {
  it('mapea el método HTTP a la acción de la matriz', () => {
    expect(actionForMethod('POST')).toBe('canCreate');
    expect(actionForMethod('put')).toBe('canUpdate');
    expect(actionForMethod('PATCH')).toBe('canUpdate');
    expect(actionForMethod('DELETE')).toBe('canDelete');
  });
});
