...
interface FinancingData {
  ...
  vehicle?:
    ...
    | { connect?: (string | { id: string; documentId?: string })[] };
}
 ...
 */
const extractVehicleIdentifier = (vehicle: any): { id?: number; documentId?: string } | null => {
  if (!vehicle) return null;

  strapi.log.info('[extractVehicleIdentifier] Input:', vehicle, 'tipo:', typeof vehicle);
 ...
 */
const getVehicleBillingInitials = async (strapi: any, vehicle: any): Promise<string | null> => {
  const identifier = extractVehicleIdentifier(vehicle);

  if (!identifier) {
    strapi.log.info('[getVehicleBillingInitials] No se pudo extraer identificador del vehículo');
    return null;
  }

  strapi.log.info('[getVehicleBillingInitials] Identificador extraído:', identifier);
  ...
  try {
    ...
    if (identifier.documentId) {
      strapi.log.info(
        '[getVehicleBillingInitials] Buscando por documentId:',
    ...
    // Buscar por ID numérico
      ...
      strapi.log.info('[getVehicleBillingInitials] Buscando por ID:', identifier.id);
      fleetVehicle = await strapi.entityService.findOne('api::fleet.fleet', identifier.id, {
        fields: ['billingInitials'],
      });
    }

    strapi.log.info('[getVehicleBillingInitials] Resultado:', fleetVehicle);
    strapi.log.info('[getVehicleBillingInitials] billingInitials:', fleetVehicle?.billingInitials);

    return fleetVehicle?.billingInitials || null;
  } catch (error) {
    strapi.log.error('[getVehicleBillingInitials] Error:', error);
    return null;
  }
};
...
export default {
  async beforeCreate(event: { params: { data: FinancingData } }) {
    ...
    strapi.log.info('[beforeCreate] Datos recibidos:', JSON.stringify(data, null, 2));
    strapi.log.info('[beforeCreate] data.vehicle:', data.vehicle);
    ...
    if (!data.financingNumber) {
      ...
      strapi.log.info('[beforeCreate] Prefijo a usar:', prefix);
      data.financingNumber = generateFinancingNumber(prefix);
      strapi.log.info('[beforeCreate] Número generado:', data.financingNumber);
    }
  ...
  async beforeUpdate(event: { params: { data: FinancingData } }) {
    ...
    if (
      ...
      data.financingMonths !== undefined
    ) {
  ...
  async beforeDelete(event: { params: { where: { documentId?: string; id?: number } } }) {
    ...
    if (!financingId) {
      strapi.log.info('[beforeDelete] No se pudo determinar el ID del financiamiento');
      return;
    }

    strapi.log.info('[beforeDelete] Eliminando pagos asociados al financiamiento ID:', financingId);

    // Buscar todos los billing-records asociados a este financiamiento
    // Usamos el ID interno para la relación
    const billingRecords = await strapi.entityService.findMany(
    ...
    if (!billingRecords || billingRecords.length === 0) {
      strapi.log.info('[beforeDelete] No hay pagos asociados para eliminar');
      return;
    }

    strapi.log.info(`[beforeDelete] Encontrados ${billingRecords.length} pagos para eliminar`);
    ...
    for (const record of billingRecords) {
      if (record.id) {
        try {
          ...
          strapi.log.info(`[beforeDelete] Pago eliminado ID: ${record.id}`);
        } catch (error) {
          strapi.log.error(`[beforeDelete] Error al eliminar pago ID ${record.id}:`, error);
          // Continuar con el siguiente aunque falle uno
        }
      }
    }

    strapi.log.info('[beforeDelete] Todos los pagos asociados han sido eliminados');
  },
};
