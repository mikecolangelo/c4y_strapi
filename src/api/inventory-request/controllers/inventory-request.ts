...
export default factories.createCoreController('api::inventory-request.inventory-request', ({ strapi }) => ({
  async find(ctx) {
    const results = await strapi.entityService.findMany('api::inventory-request.inventory-request', {
  ...
  async findOne(ctx) {
    ...
    const result = await strapi.entityService.findOne('api::inventory-request.inventory-request', id, {
      populate: ['requester', 'approvedBy', 'inventoryItem'],
    }) as any;
  ...
  async approve(ctx) {
    ...
    const updatedRequest = await strapi.db.query('api::inventory-request.inventory-request').update({
      where: { id },
      data: {
        status: 'aprobado',
