  ...
  async syncReminderNotifications(reminder: any) {
    try {
      ...
      if (reminder.id && typeof reminder.id === 'number') {
        ...
        const allNotifications: any[] = await strapi.entityService.findMany(
          'api::notification.notification',
          {
      ...
      if (fullReminder.module === 'fleet' && fullReminder.fleetVehicle) {
        const vehicleName = (fullReminder.fleetVehicle as any)?.name || 'Vehículo';
