...
async function validateDocumentType(
  ...
  try {
    const documentType = await strapi.entityService.findOne(
  ...
  ({ strapi }) => ({
    ...
    async getDocumentTypes(ctx) {
      try {
        ...
        strapi.log.error('Error fetching document types:', error);
    ...
    async create(ctx) {
      ...
      if (
        !data?.authorDocumentId ||
        data.authorDocumentId === null ||
        data.authorDocumentId === undefined
      ) {
      ...
      try {
        ...
        strapi.log.error('Error creating fleet-document via db.query:', error);
    ...
    async update(ctx) {
      ...
      const vehicleId =
        data?.vehicle !== undefined && data?.vehicle !== null
          ? extractRelationId(data.vehicle)
          : undefined;
      ...
      try {
        ...
        strapi.log.error('Error updating fleet-document via db.query:', error);
