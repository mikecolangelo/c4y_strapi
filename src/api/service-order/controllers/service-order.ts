...
function normalizeRelations(data: any) {
  ...
  if (result.services !== undefined) {
    if (result.services === null || (Array.isArray(result.services) && result.services.length === 0)) {
      result.services = { set: [] };
  ...
  if (result.inventoryItems !== undefined) {
    if (result.inventoryItems === null || (Array.isArray(result.inventoryItems) && result.inventoryItems.length === 0)) {
      result.inventoryItems = { set: [] };
    } else if (Array.isArray(result.inventoryItems)) {
      result.inventoryItems = { connect: result.inventoryItems.map((id: any) => ({ id: Number(id) })) };
    }
  }
...
async function createUsedItems(strapi: any, orderId: number, usedItems: any[]) {
  ...
  for (const item of usedItems) {
    ...
    if (!Number.isFinite(resolvedInventoryItem) || resolvedInventoryItem <= 0) {
      console.warn(`[createUsedItems] Skipping item with invalid inventoryItem:`, item.inventoryItem);
      continue;
    }
...
async function removeExistingUsedItems(strapi: any, orderId: number) {
  const existing = await strapi.db.query('api::service-order-inventory-item.service-order-inventory-item').findMany({
    where: { serviceOrder: orderId },
    select: ['id'],
  });
  if (existing && existing.length > 0) {
    for (const row of existing) {
      await strapi.db.query('api::service-order-inventory-item.service-order-inventory-item').delete({
        where: { id: row.id },
      });
    }
  }
...
async function fetchUsedItems(strapi: any, orderId: number) {
  const rows = await strapi.db.query('api::service-order-inventory-item.service-order-inventory-item').findMany({
    where: { serviceOrder: orderId },
...
async function finalizeOrder(ctx: any, strapi: any, numericId: number, order: any, updateData: any) {
  ...
  for (const item of usedItems || []) {
    ...
    await createInventoryMovement(strapi, {
      type: 'salida',
  ...
  ({ strapi }) => ({
    ...
    async update(ctx) {
      ...
      if (usedItems !== undefined) {
        ...
        const laborCost = parseFloat(
          data.laborCost ?? existingOrder.laborCost ?? 0
        );
      ...
      const finalPartsCost = partsCost !== null ? partsCost : parseFloat(existingOrder.partsCost || 0);
    ...
    async createFromMaintenance(ctx) {
      ...
      try {
        ...
        if (!maintenanceKit) {
          return ctx.notFound(`No existe un kit de mantenimiento activo para el tipo: ${maintenanceType}`);
        }
        ...
        const result = await knex.transaction(async (trx) => {
          ...
          const [orderRow] = await knex('service_orders')
            ...
            .insert({
              ...
              scheduled_at: now,
              completed_at: now,
              summary: notes || `Orden generada automáticamente desde kit de mantenimiento: ${maintenanceKit.name}`,
          ...
          await knex('service_orders_vehicle_lnk')
            .transacting(trx)
            .insert({
          ...
          if (driverId) {
            await knex('service_orders_driver_lnk')
              .transacting(trx)
              .insert({
                service_order_id: orderId,
          ...
          if (maintenanceKit.service?.id) {
            try {
              ...
              strapi.log.warn('[createFromMaintenance] No se pudo vincular servicio del catálogo:', (lnkError as Error).message);
            }
          }
